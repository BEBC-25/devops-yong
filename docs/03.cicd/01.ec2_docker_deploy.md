# 1. mybatis-sns 컨테이너라이징 및 AWS EC2 배포 실습

## 학습 목표
- Java 25 및 Spring Boot 4 기반 mybatis-sns 애플리케이션의 최적화된 Dockerfile 작성 및 단일 컨테이너 로컬 구동 검증
- Docker Compose를 활용하여 mybatis-sns 앱과 mybatis-sns-db(MySQL) 다중 컨테이너를 로컬 환경에서 통합 배포 및 검증
- Docker Hub 원격 레지스트리 준비, 개인 액세스 토큰(PAT) 발급 및 로컬 인증 로그인
- mybatis-sns 도커 이미지 빌드, 태깅 및 Docker Hub 원격 푸시
- AWS EC2(amazonlinux-docker AMI 기반) 인스턴스 환경 점검 및 SSH 원격 접속
- SCP를 통한 민감 환경 변수(.env) 및 Docker Compose 설정 파일의 안전한 배포
- EC2 운영 환경에서의 Docker Compose 서비스 기동 및 외부 접속 검증
- 소스 코드 변경 시 7단계 반복 배포 프로세스 분석 및 CI/CD 자동화 전환의 필요성 이해

## 목차
- [1. mybatis-sns 컨테이너라이징 및 로컬 서비스 검증](#1-mybatis-sns-컨테이너라이징-및-로컬-서비스-검증)
  - [1.1 컨테이너라이징 사전 작업 (환경 변수 및 시크릿 분리)](#11-컨테이너라이징-사전-작업-환경-변수-및-시크릿-분리)
  - [1.2 Dockerfile 최적화 및 보안 설계 원칙](#12-dockerfile-최적화-및-보안-설계-원칙)
  - [1.3 경량 멀티 스테이지 Dockerfile 작성](#13-경량-멀티-스테이지-dockerfile-작성)
  - [1.4 Docker Compose 기반 mybatis-sns-app 및 mybatis-sns-db 로컬 통합 배포](#14-docker-compose-기반-mybatis-sns-app-및-mybatis-sns-db-로컬-통합-배포)
- [2. Docker Hub 푸시 및 AWS EC2 배포 실습](#2-docker-hub-푸시-및-aws-ec2-배포-실습)
  - [2.1 Docker Hub 원격 레지스트리 준비 및 로컬 로그인](#21-docker-hub-원격-레지스트리-준비-및-로컬-로그인)
  - [2.2 mybatis-sns 이미지 빌드, 태깅 및 Docker Hub 푸시](#22-mybatis-sns-이미지-빌드-태깅-및-docker-hub-푸시)
  - [2.3 AWS EC2 인스턴스 생성](#23-aws-ec2-인스턴스-생성)
  - [2.4 키 파일 복사 및 Git 보안 관리 (.gitignore 설정)](#24-키-파일-복사-및-git-보안-관리-gitignore-설정)
  - [2.5 EC2 인스턴스 원격 SSH 접속](#25-ec2-인스턴스-원격-ssh-접속)
  - [2.6 EC2에 Docker Engine 및 Docker Compose 설치](#26-ec2에-docker-engine-및-docker-compose-설치)
  - [2.7 EC2 서버 환경 변수(.env) 및 Docker Compose 설정 파일 배치](#27-ec2-서버-환경-변수env-및-docker-compose-설정-파일-배치)
  - [2.8 EC2 상에서 Docker Compose 서비스 기동 및 동작 검증](#28-ec2-상에서-docker-compose-서비스-기동-및-동작-검증)
  - [2.9 소스 코드 변경에 따른 재배포 흐름과 CI/CD 전환 필요성](#29-소스-코드-변경에-따른-재배포-흐름과-cicd-전환-필요성)

---

# 1. mybatis-sns 컨테이너라이징 및 로컬 서비스 검증

## 1.1 컨테이너라이징 사전 작업 (환경 변수 및 시크릿 분리)
### 1.1.1 환경 변수 주입 방식
...
### 1.1.3 IntelliJ IDEA 로컬 개발 환경 적용
...
## 1.3 경량 멀티 스테이지 Dockerfile 작성
- 최적화/보안 설계 원칙과 사전 준비된 환경 변수 설정을 바탕으로 멀티 스테이지 Dockerfile을 작성.

### 1.3.1 경량 멀티 스테이지 Dockerfile 명세
...
## 1.4 Docker Compose 기반 mybatis-sns-app 및 mybatis-sns-db 로컬 통합 배포
...
### 1.4.2 로컬 개발용 Docker Compose 명세 (`docker-compose.yaml`)
...
### 1.4.3 로컬 환경 변수 파일 (.env) 동기화
- 환경 변수 적용 우선순위
  - 컨테이너 구동 및 Spring Boot 설정 바인딩은 아래와 같은 우선순위를 따름.
    `CLI (-e)` > `environment` 속성 > `env_file` 속성 > `Dockerfile (ENV)` > `application.yaml (기본값)`
...
# 2. Docker Hub 푸시 및 AWS EC2 배포 실습

## 2.1 Docker Hub 원격 레지스트리 준비 및 로컬 로그인
...
### 2.1.1 Docker Hub 개인 액세스 토큰(PAT) 발급
- Docker Hub 공식 사이트(https://hub.docker.com)에 로그인.
...
### 2.1.2 로컬 터미널 Docker Hub 인증 로그인
...
### 2.1.3 GitHub Secrets에 Docker Hub 자격 증명 사전 등록
...
- 웹 브라우저에서 hub.docker.com에 접속한 후 Repositories 메뉴에서 `mybatis-sns` 리포지토리를 선택하고 Tags 탭에 `latest`, `v1.0.0` 태그가 등록되었는지 확인.

### 2.2.3 로컬 Docker Compose 배포 이미지 수정
...
## 2.3 AWS EC2 인스턴스 생성
- AWS 단원에서 제작한 amazonlinux-docker AMI(Amazon Machine Image) 기반의 EC2 인스턴스를 활용하여 배포.

### 2.3.1 EC2 서비스 이동 및 대시보드 접근
- EC2 서비스 이동
...
## 2.4 키 파일 복사 및 Git 보안 관리 (.gitignore 설정)
...
## 2.6 EC2에 Docker Engine 및 Docker Compose 설치
- SSH로 EC2에 접속한 상태에서 Docker Engine 및 Docker Compose 플러그인을 일괄 설치하고 서비스를 활성화.

### 2.6.1 Docker Engine 및 Docker Compose 플러그인 일괄 설치
```bash
...
## 2.7 EC2 서버 환경 변수(.env) 및 Docker Compose 설정 파일 배치
...
## 2.8 EC2 상에서 Docker Compose 서비스 기동 및 동작 검증
- Docker Hub에 업로드된 이미지를 EC2 서버로 내려받아 전체 서비스를 기동하고 외부 접속을 검증함.

### 2.8.1 원격 이미지 다운로드 및 백그라운드 서비스 기동
...
### 2.8.2 컨테이너 상태 및 스프링 부트 구동 로그 점검
...
### 2.8.3 웹 브라우저 및 curl 기반 서비스 응답 테스트
...
## 2.9 소스 코드 변경에 따른 재배포 흐름과 CI/CD 전환 필요성
- 실제 서비스 개발 과정에서는 기능 추가, UI 수정, 버그 패치 등으로 인해 소스 코드가 빈번하게 수정됨.

### 2.9.1 소스 코드 변경 시 거쳐야 하는 반복 작업 절차
...
#### 1) 1단계: 로컬 테스트 및 JAR 빌드 검증 (1장 영역)
...
### 2.9.2 반복 프로세스의 한계와 CI/CD 자동화의 핵심 가치
