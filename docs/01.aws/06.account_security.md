...
- Windows 환경
  - 파워셸(PowerShell) 실행 후 최신 v2 패키지 설치 진행.
```powershell
...
```
- 샘플 테스트 파일 생성 및 S3 버킷 업로드 시도
```bash
echo "test file content" > sample.txt
aws s3 cp sample.txt s3://my-bucket/uploads/sample.txt
```
- Presigned URL 생성 시도(5분)
```bash
aws s3 presign s3://my-bucket-213470584918-ap-south-1-an/uploads/sample.txt --expires-in 300
```
