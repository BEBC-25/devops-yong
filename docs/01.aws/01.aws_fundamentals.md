...
- [3. AWS 계정 생성 및 EC2 인스턴스 생성 실습](#3-aws-계정-생성-및-ec2-인스턴스-생성-실습)
  ...
  - [3.4 키 파일 복사 및 Git 보안 관리 (.gitignore 설정)](#34-키-파일-복사-및-git-보안-관리-gitignore-설정)
  - [3.5 EC2 인스턴스 원격 SSH 접속](#35-ec2-인스턴스-원격-ssh-접속)
  - [3.6 Java 25 런타임 환경 (Temurin JDK) 설치](#36-java-25-런타임-환경-temurin-jdk-설치)
  - [3.7 개발 프로젝트 빌드 및 EC2 파일 전송](#37-개발-프로젝트-빌드-및-ec2-파일-전송)
  - [3.8 애플리케이션 직접 실행 및 접속 테스트](#38-애플리케이션-직접-실행-및-접속-테스트)
  - [3.9 리눅스 패킷 포워딩 및 포트 재전송 (iptables)](#39-리눅스-패킷-포워딩-및-포트-재전송-iptables)
  - [3.10 애플리케이션 무중단 백그라운드 실행](#310-애플리케이션-무중단-백그라운드-실행)
...
<img src="../images/aws/01-01.png" alt="클라우드의 주요 이점" width="800" />
...
<img src="../images/aws/01-02.png" alt="AWS 핵심 서비스 범주" width="800" />
...
<img src="../images/aws/01-04.png" alt="AWS 글로벌 인프라 구조" width="800" />
...
<img src="../images/aws/01-03.png" alt="AWS 데이터 센터 인프라" width="800" />
...
<img src="../images/aws/01-05.png" alt="AWS 글로벌 리전 현황" width="800" />
...
<img src="../images/aws/01-06.png" alt="엣지 로케이션 캐싱 및 데이터 전송 구조" width="800" />
...
## 3.4 키 파일 복사 및 Git 보안 관리 (.gitignore 설정)
...
## 3.6 Java 25 런타임 환경 (Temurin JDK) 설치
...
### 3.6.4 Temurin Java 25 JDK 패키지 설치
...
## 3.7 개발 프로젝트 빌드 및 EC2 파일 전송

### 3.7.1 Spring Boot 애플리케이션 빌드
...
### 3.7.2 EC2 서버에 빌드 결과물 전송
- git bash 환경에서 실행

```sh
# 빌드 파일명이 spring-board-0.0.1-SNAPSHOT.jar이고 인스턴스의 퍼블릭 IP가 12.34.56.78일 경우의 예시
scp -i "first-key.pem" build/libs/spring-board-0.0.1-SNAPSHOT.jar ec2-user@12.34.56.78:/home/ec2-user/
```

## 3.8 애플리케이션 실행 및 접속 테스트

### 3.8.1 스프링 부트 애플리케이션 실행
...
### 3.8.2 보안 그룹에 8080번 포트 인바운드 규칙 추가
- 보안 그룹 콘솔로 이동
...
## 3.9 리눅스 패킷 포워딩 및 포트 재전송 (iptables)

### 3.9.1 iptables 방화벽 서비스 설치 및 데몬 활성화
...
# 서비스 자동 시작 등록 및 즉시 구동
sudo systemctl enable --now iptables
```

### 3.9.2 80번 HTTP 포트를 8080번 애플리케이션 포트로 리다이렉트
...
### 3.9.3 방화벽 체인 및 NAT 리다이렉트 설정 규칙 검증
...
### 3.9.4 보안 그룹 8080번 포트 인바운드 규칙 제거
...
## 3.10 애플리케이션 무중단 백그라운드 실행

### 3.10.1 백그라운드 무중단 실행
- 터미널 포그라운드 프로세스 종료
  - SSH 터미널 창에서 Ctrl + C 키를 눌러 기존 포그라운드 실행 중지
- nohup(No Hang Up) 백그라운드 무중단 구동 및 로그 저장
  - SSH 접속 종료 후에도 상시 동작하도록 백그라운드 모드로 실행하고 출력 로그를 app.log 파일로 누적 저장

```sh
# nohup: SSH 터미널 접속 종료 시 발생하는 SIGHUP 시그널을 무시하여 상시 무중단 구동
# > app.log: 표준 출력을 app.log 파일로 저장
# 2>&1: 표준 에러를 표준 출력으로 통합
# &: 프로세스를 터미널 백그라운드 작업으로 전환
nohup java -jar spring-board-0.0.1-SNAPSHOT.jar > app.log 2>&1 &
```

- nohup (No Hang Up) 개념 및 동작 원리
  - Hang Up(전화 수화기를 끊음) 어원에서 유래하여, SSH 터미널 세션 연결이 끊어질 때 리눅스 셸이 전송하는 `SIGHUP`(Signal Hang Up, 세션 종료 신호) 시그널을 프로세스가 무시하도록 차단하는 역할 수행
- nohup 키워드 미사용 시 차이점
  - `java -jar ... &`만 실행 시: 현재 터미널이 열려 있는 동안만 백그라운드로 구동되며, SSH 터미널 창을 닫는 순간 `SIGHUP`(연결 끊김 신호)을 전달받아 프로세스가 자동 종료됨
  - `nohup java -jar ... &` 실행 시: SSH 터미널 세션이 종료되어도 `SIGHUP` 신호를 무시하므로 서버에서 무중단 상시 운영 유지 가능

### 3.10.2 백그라운드 프로세스 조회 및 종료
- 백그라운드로 실행중인 프로스세를 종료하기 위해서는 프로세스 ID(PID)를 찾아서 강제 종료해야 함

#### 1) 실행 중인 자바 백그라운드 프로세스 ID(PID) 확인
- `ps` 명령어 또는 `pgrep` 명령어를 활용하여 구동 중인 스프링 부트 애플리케이션의 PID 확인

- 실행 중인 Java 프로세스 목록 조회 및 PID 확인
```sh
ps -ef | grep java
```

- 출력 예시 (UID: 소유 계정, PID: 프로세스 ID, PPID: 부모 프로세스 ID, C: CPU 사용 지수, STIME: 시작 시간, TTY: 접속 터미널, TIME: CPU 사용 시간, CMD: 실행 명령)
```sh
UID        PID  PPID  C STIME TTY          TIME CMD
ec2-user 28100 12516 13 14:40 pts/0    00:00:13 java -jar spring-board-0.0.1-SNAPSHOT.jar
ec2-user 28146 28073  0 14:42 pts/1    00:00:00 grep --color=auto java
```

- 특정 jar 파일명 기반 PID 검색
```sh
pgrep -fl spring-board
```
...
#### 2) 백그라운드 프로세스 종료 (kill)
- 1단계에서 확인한 프로세스 ID(PID) 번호를 지정하여 백그라운드 프로세스 종료

```sh
# 일반 프로세스 종료 요청 (PID 12345 예시)
kill 12345

# 프로세스가 정상적으로 종료되지 않는 경우 강제 종료
kill -9 12345
```
