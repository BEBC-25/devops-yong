...
- [4. spring-board, Nginx, MySQL, Redis 기반 다중 컨테이너 구축 실습](#4-spring-board-nginx-mysql-redis-기반-다중-컨테이너-구축-실습)
  ...
  - [4.6 도전!!! 초기 접속시 502 Bad Gateway 에러를 잡아라!](#46-도전-초기-접속시-502-bad-gateway-에러를-잡아라)
...
- 세션 공유 정상 동작 확인
  ...
  - 로그인 완료 후 `F5` 새로고침을 하면 요청이 `board-app-1`과 `board-app-2`로 번갈아 분산되더라도, 두 WAS가 중앙 `redis-cache` 세션 스토리지에서 동일한 세션을 공유하므로 로그인 상태가 끊어지지 않고 유지됨.

- Redis CLI를 통한 세션 데이터 영속 저장 확인
  - `redis-cli` 도구를 사용하여 Redis 내부에 실제 스프링 세션 키(`spring:session:sessions:`)가 정상 생성되어 저장되었는지 직접 확인.
  ```bash
  docker exec -it redis-cache redis-cli
  ```
  ```redis
  127.0.0.1:6379> keys *
  1) "spring:session:sessions:a8f9c73e-3b2a-4f5e-9c8d-..."
  ```
...
- Nginx 서비스(`web-lb`)의 `depends_on` 의존 대상을 기존 `board-app-1`, `board-app-2`에서 단일 서비스인 `- board-app`으로 수정.
...
- 명령줄에서 지정한 수량 만큼 확장(Scale-out) 구동.
  ```bash
  docker compose -f docker/docker-compose.yml up -d --force-recreate --remove-orphans --scale board-app=3
  ```
...
### 4.6 도전!!! 초기 접속시 502 Bad Gateway 에러를 잡아라!

- 문제 증상 파악
  - `docker compose -f docker/docker-compose.yml up -d` 명령어로 모든 인프라를 구동한 직후, 즉시 브라우저에서 `http://localhost` 에 접속하면 Nginx의 `502 Bad Gateway` 에러 화면이 약 10~15초간 노출되는 현상 발생.
  - 구동 후 약 10~15초가 지나야 비로소 정상적인 게시판 홈 화면이 켜짐.

- 원인 진단
  - Nginx 컨테이너(`web-lb`)는 구동 후 1초 만에 초기화가 완료되어 80 포트로 HTTP 요청 수신을 즉시 시작함.
  - 반면 Spring Boot WAS 컨테이너(`board-app`)는 JVM 부트스트랩, 스프링 컨텍스트 로딩 및 커넥션 풀 구성에 약 10~15초의 시간이 소요됨.
  - Nginx 서비스 명세의 `depends_on: - board-app` 설정은 단순 컨테이너 시작(`service_started`) 상태만 판별할 뿐, WAS 내부 스프링부트가 8080 포트 수신 준비(Ready)를 마쳤는지까지는 대기해주지 않기 때문에 불완전한 502 응답이 유발됨.

- 트러블슈팅 해결 과제
  1. WAS 서비스(`board-app`)에 헬스체크(`healthcheck`) 명세 정의
     - WAS 내부에서 8080 포트 응답 가능 여부를 주기적으로 검증하는 헬스체크 테스트 구문 작성.
  2. Nginx 서비스(`web-lb`)의 의존성 조건 강화
     - Nginx 서비스의 `depends_on` 의존 조건을 단순 `service_started`에서 `condition: service_healthy`로 교체하여 WAS 헬스체크 통과 이후 Nginx가 트래픽 수신을 시작하도록 제어.
